...
class CategoryRemoteWorkerTests: XCTestCase {
    ...
    func test_success_response() throws {
        let result = worker.treatResponse(response: MokeAPIResponse.categorySuccessResult)
        switch result {
    ...
    func test_failure_response_errorResultDataFailed() throws {
        let result = worker.treatResponse(response: MokeAPIResponse.categoryErrorResultDataFailed)
        switch result {
        case .success(_):
    ...
    func test_failure_response_errorResultInvalidURL() throws {
        let result = worker.treatResponse(response: MokeAPIResponse.categoryErrorResultInvalidURL)
        switch result {
        case .success(_):
            XCTFail("should be failure insted of success")
        case .failure(let error):
    ...
    func test_failure_response_errorResultParsingError() throws {
        let result = worker.treatResponse(response: MokeAPIResponse.categoryErrorResultParsingError)
        switch result {
        case .success(_):
            XCTFail("should be failure insted of success")
        case .failure(let error):
